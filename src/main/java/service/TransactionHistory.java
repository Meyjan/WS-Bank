...
public class TransactionHistory {
    ...
    public History[] AccountHistory(Integer account) {
        ...
        try {
            ...
            String query = "SELECT * FROM account_transaction_tbl WHERE transaction_type = \"DEBIT\" AND (account_transaction_tbl.account_number = "
                    + account + " OR account_transaction_tbl.target_account = " + account + ")";
    ...
    public boolean CheckCredit(Integer account, Integer amount, String start, String end) {
        ...
        try {
            ...
            Statement st = conn.createStatement();
            String query = "SELECT * FROM account_transaction_tbl WHERE amount = " + amount
                    + " AND transaction_time BETWEEN " + start + " AND " + end + " AND ((account_number = " + account
                    + " AND transaction_type = \"KREDIT\") OR (target_account = " + account
