...
public class TransactionHistory {
    ...
    public boolean CheckCredit(Integer account, Integer amount, String start, String end) {
        ...
        try {
            ...
            Statement st = conn.createStatement();
            String query = "SELECT * FROM account_transaction_tbl WHERE account_number = " 
                            + account + " AND amount = " + amount + " AND transaction_type = " + "\'KREDIT\'" + " AND transaction_time BETWEEN " 
                            + start + " AND " + end;
            ResultSet rawResult = st.executeQuery(query);
